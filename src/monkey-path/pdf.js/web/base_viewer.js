...
import { createPromiseCapability } from "pdfjs-dist/webpack";
import {
  CSS_UNITS,
  DEFAULT_SCALE,
  DEFAULT_SCALE_VALUE,
  ...
  VERTICAL_PADDING,
  watchScroll,
} from "pdfjs-dist/lib/web/ui_utils.js";
import { PDFRenderingQueue, RenderingStates } from "pdfjs-dist/lib/web/pdf_rendering_queue.js";
import { AnnotationLayerBuilder } from "pdfjs-dist/lib/web/annotation_layer_builder.js";
import { PDFPageView } from "pdfjs-dist/lib/web/pdf_page_view.js";
import { SimpleLinkService } from "pdfjs-dist/lib/web/pdf_link_service.js";
    ...
    firstPagePromise
      .then(firstPdfPage => {
        ...
        for (let pageNum = 1; pageNum <= pagesCount; ++pageNum) {
          ...
          const settingPage = window.PDFViewerApplication.settingPages[pageNum - 1];
          if (settingPage) {
            if (
              settingPage?.embed?.pdfSvg ||
              (settingPage?.style?.embed?.type === "video" && settingPage?.style?.embed?.url)
            ) {
